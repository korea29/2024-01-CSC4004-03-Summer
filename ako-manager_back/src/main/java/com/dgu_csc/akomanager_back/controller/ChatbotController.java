    ...
    public ResponseEntity<ChatResponse> askChatbot(@RequestBody ChatRequest chatRequest, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            switch (response) {
                // 0번, 1번 : 아코 매니저 자기 소개
                case "0" : {
                    response = "안녕하세요! 아코 매니저입니다~ 원하시는 질문이 있으시다면 물어봐 주세요!";
                    break;
                }
                case "1" : {
                    response = "안녕하세요~ 저는 동국대학교 학생들을 돕기 위해 이 세상에 태어난 아코 매니저입니다!";
                    break;
                }
                // 2번 : 수강해야 하는 과목들에 대한 정보 물었을 때
                case "2" : {
                    response = username + "님이 수강해야 하는 과목들에 대한 정보를 불러왔어요";
                    break;
                }
                // 3번 : 전공 졸업기준표를 요구하는 경우, 수강해야 하는 교양 필수, 전공 필수 학점 알려 주기
                case "3" : {
                    response = username + "님이 다니시는 전공의" + " 교양 필수 과목은 ["
                            + majorService.getTotalDesignatedScore(majorDto) + "]학점, 전공 필수 학점은 ["
                            + majorService.getTotalMajorScore(majorDto) + "]학점을 포함하여 총 ["
                            + majorService.getTotalScore(majorDto) + "]학점을 수강해야 합니다";
                    break;
                }
                // 4번 : 추천 시간표 요구
                case "4" : {
                    response = username + "님이 수강해야 하는 과목들을 기준으로 추천 시간표를 짜 봤어요";
                    break;
                }
                // 5번 : 해결됐다고 했을 때
                case "5" : {
                    response = "해결되었다니 정말 다행이네요! 언제든 도움이 필요하시다면 아코 매니저를 불러 주세요!";
                    break;
                }
                // 6번 : 선이수 과목 요구
                case "6" : {
                    response = "해당 과목을 듣기 위해서 추가로 수강해야 하는 과목들을 가지고 왔어요";
                    break;
                }
                // 7번 : 졸업 조건 만족을 위해 더 들어야 하는 학점 정보
                case "7" : {
                    response = "졸업 기준을 만족하기 위해 "+username+"님은 [" + (majorService.getTotalScore(majorDto) - subjectFinishedServiceImpl.getUserTotalScore(chatResponse.getStudentId()))
                ...
                case "8" : {
                    response = username + "님은 전공 [" + subjectFinishedServiceImpl.getUserTotalMajorScore(chatResponse.getStudentId()) + "] 학점, 교양 [" + subjectFinishedServiceImpl.getUserTotalCommonScore(chatResponse.getStudentId()) +
                ...
                case "10" : {
                    response = username + "님은 전공 [" + subjectFinishedServiceImpl.getUserTotalMajorScore(chatResponse.getStudentId()) + "] 학점을 이수했습니다";
                    break;
                }
                // 11번 : 전공 중 추가로 들어야 하는 학점 수를 요구하는 경우
                case "11" : {
                    response = "전공은 [" + (majorService.getTotalMajorScore(majorDto) - subjectFinishedServiceImpl.getUserTotalMajorScore(chatResponse.getStudentId())) + "] 학점을 더 이수해야 합니다";
                    break;
                }
                // 12번 : 미 수강한 교양 필수 학점 수를 요구하는 경우
                case "12" : {
                    response = "교양 필수는 [" + (majorService.getTotalDesignatedScore(majorDto) + majorService.getTotalCommonScore(majorDto) - subjectFinishedServiceImpl.getUserTotalCommonScore(chatResponse.getStudentId())) + "]학점을 필수적으로 들어야 합니다";
                    break;
                }
                // 13번 : 총 전공 학점 수를 요구
                case "13" : {
                    response = username + "님의 전공은 [" + majorService.getTotalMajorScore(majorDto) + "]학점을 전공으로 채워야 합니다";
                    break;
                }
                // 14번 : 총 요구하는 최소 교양 학점 수를 요구
                case "14" : {
                    response = username + "님의 전공은 [" + (majorService.getTotalDesignatedScore(majorDto) + majorService.getTotalCommonScore(majorDto)) + "]학점을 교양 필수으로 채워야 합니다";
                    break;
                }
            }
            chatBot.setBotinput(response);
