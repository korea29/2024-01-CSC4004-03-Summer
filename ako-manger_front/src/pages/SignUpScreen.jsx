...
const SignUpScreen = () => {
  ...
  const [selectedFileName, setSelectedFileName] = useState('수강과목 첨부(.xls)*'); // 선택된 파일명
  ...
  const handleFileInputChange = (e) => { // 프로필
    setFormData({ ...formData, profileImage: e.target.files[0] }); // 사진 업로드
  };
  const handleExcelInputChange = (e) => { // 엑셀 파일
  ...
  const handleStudentIdChange = (e) => { // 학번
    const value = e.target.value;
    if (/^\d*$/.test(value) || value === '') { // 숫자 형식 또는 빈 문자열인 경우에만 입력을 허용
      setFormData({ ...formData, studentId: value });
    }
  };
  
  const handleNameChange = (e) => { // 이름
    const value = e.target.value;
    if (/^[a-zA-Z가-힣\s]*$/.test(value) || value === '') { // 영문자 및 한글 형식 또는 빈 문자열인 경우에만 입력을 허용
      setFormData({ ...formData, name: value });
    }
  };
  
  const handleMajorChange = (e) => { // 전공
    const value = e.target.value;
    if (/^[a-zA-Z가-힣\s]*$/.test(value) || value === '') { // 영문자 및 한글 형식 또는 빈 문자열인 경우에만 입력을 허용
  ...
  const handleSecondMajorChange = (e) => { //부전공
    const value = e.target.value;
    if (/^[a-zA-Z가-힣\s]*$/.test(value) || value === '') { // 영문자 및 한글 형식 또는 빈 문자열인 경우에만 입력을 허용
  ...
  const handleStudentBirthChange = (e) => { // 생년월일
    const value = e.target.value;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.university || !formData.studentId || !formData.name || !formData.major || !formData.college ||!formData.excelFile) { // 생년월일 필수로 수정하면 추가해주기
  ...
  return (
    <div className="signup-container">
      ...
      <div className="profile-container">
        ...
        <div className="profile-text" onClick={() => inputFileRef_2.current.click()}>Profile</div>
        <input
          type="file"
          accept=".png,.jpg"
          ref={inputFileRef_2}
      ...
      <form className="signup-form" onSubmit={handleSubmit}>
        ...
        />
      <div className="file-upload-container" > {/* 수강과목 첨부 */} 
      {selectedFileName && <div className="file-upload-message">{selectedFileName} 
          <button className="attach-file-button" onClick={(e) => { e.preventDefault(); inputFileRef.current.click(); }}>첨부하기</button>
          <input
            type="file"
            accept=".xls,.xlsx"
            ref={inputFileRef}
      ...
      <div className="help" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} style={{ position: 'absolute', top: '0', left: '0' }}>
        도움말 {messageVisible && <p className="helptext" style={{ position: 'absolute', top: '0', left: '100%', marginLeft: '10px' }}> 'mdrims - 졸업 대상자 관리 - 취득학점확인서 조회'에서 .xls 다운로드</p>}
    </div>
      </div>
